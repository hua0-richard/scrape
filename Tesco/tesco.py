...
def scrapeSite_tesco(driver, EXPLICIT_WAIT_TIME, idx=None, aisle='', ind=None):
    items = []
    ...
    try:
        ...
        while True:
            ...
            for _ in range(3):
                try:
                    ul_element = wait.until(EC.presence_of_element_located((By.ID, "list-content")))
                    li_elements = ul_element.find_elements(By.CLASS_NAME, "LD7hL")
                    for li in li_elements:
                        product_link = li.find_element(By.CSS_SELECTOR, "a.ddsweb-link__anchor")
                        href = product_link.get_attribute("href")
                        items.append(href)
                    print(len(items))
                    break
                except Exception as e:
                    print(f'Failed to get links... trying again attempt {_}')

            try:
                next_button = WebDriverWait(driver, 10).until(
                    EC.element_to_be_clickable((By.CSS_SELECTOR, "a[data-next-previous-btn='next']"))
                )
                next_button.click()
